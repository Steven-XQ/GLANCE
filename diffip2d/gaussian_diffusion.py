...
class GaussianDiffusion:
    ...
    def q_sample(self, x_start, t, noise=None, mask=None):
        ...
        if mask == None:
            ...
            return th.where(mask==0, x_start, x_t) # anchor
    ...
    def p_sample(
        self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None,
            top_p=None, mask=None, x_start=None, motion_feat_encoded=None,
    ):
        """
    ...
    def p_sample_loop(
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            final_l.append(sample[1]['sample'])
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            sample_x_l = th.randn(*shape, device=device)
        ...
        for i in indices: # from T to 0
            ...
            mask_r = None
            mask_l = None
            if not (valid_mask[0]==None):
                mask_r = valid_mask[0].to(t.device)
            if not (valid_mask[1]==None):
                mask_l = valid_mask[1].to(t.device)
            with th.no_grad():
                out_r = self.p_sample(
                    model,
                    sample_x_r,
    ...
    def training_losses_seq2seq(self, model, post_model, rl_x_start, t, rl_valid_mask, motion_feat_encoded, model_kwargs=None, noise=None, gaze_feat_encoded=None):
        ...
        valid_mask_r = None
        valid_mask_l = None
        if not (rl_valid_mask[0]==None):
            valid_mask_r = rl_valid_mask[0].to(t.device)  
        if not (rl_valid_mask[1]==None):
            valid_mask_l = rl_valid_mask[1].to(t.device)

        future_mask = th.zeros(x_start_r.shape[:2]).to(t.device)
        future_mask[:,-4:] = 1

        x_start_mean_r = x_start_r
        x_start_mean_l = x_start_l  
        ...
        x_start_l = self._get_x_start(x_start_mean_l, std)

        noise_r = th.randn_like(x_start_r)
        noise_l = th.randn_like(x_start_l)


        x_t_r = self.q_sample(x_start_r, t, noise=noise_r, mask=future_mask)
        x_t_l = self.q_sample(x_start_l, t, noise=noise_l, mask=future_mask)
        ...
        assert model_output_l.shape == target_l.shape == x_start_l.shape

        loss_recon_r = mean_flat((target_r - model_output_r) ** 2)
        loss_recon_l = mean_flat((target_l - model_output_l) ** 2)

        model_out_x_start_r = self._x0_helper(model_output_r, x_t_r, t)['pred_xstart']
        model_out_x_start_l = self._x0_helper(model_output_l, x_t_l, t)['pred_xstart']
        t0_mask = (t == 0)
        t0_loss_r = mean_flat((x_start_mean_r - model_out_x_start_r) ** 2)
        t0_loss_l = mean_flat((x_start_mean_l - model_out_x_start_l) ** 2)
        loss_recon_r = th.where(t0_mask, t0_loss_r, loss_recon_r)
        loss_recon_l = th.where(t0_mask, t0_loss_l, loss_recon_l)

        terms["mse_r"] = loss_recon_r
        terms["mse_l"] = loss_recon_l
        ...
        out_mean_l, _, _ = self.q_mean_variance(x_start_l, th.LongTensor([self.num_timesteps - 1]).to(x_start_l.device))
        tT_loss_l =  mean_flat(out_mean_l ** 2)
        
        terms["tT_loss_r"] = tT_loss_r
        terms["tT_loss_l"] = tT_loss_l

        return {"loss_feat_level": terms, "rec_feature_r": model_out_x_start_r, "rec_feature_l": model_out_x_start_l}
...
class HOIDiffusion(GaussianDiffusion):
    """
