def add_nets_opts(parser):
    ...
    parser.add_argument("--gaze_coord_only", action="store_true", help="Use only raw (x,y) coordinates for gaze (skip heatmap CNN)")
    parser.add_argument("--gaze_fusion_only", action="store_true", help="Use gaze only in SideFusionEncoder gating, not in MADT cross-attention")
    parser.add_argument("--gaze_alpha_clamp", default=0.0, type=float, help="Clamp gaze_alpha max value (0=no clamp)")
    parser.add_argument("--gaze_last_n_blocks", default=0, type=int, help="Only use gaze in last N MADT blocks (0=all blocks)")
    parser.add_argument("--gaze_detach_diffusion", action="store_true", help="Detach gaze features before passing to diffusion/MADT")
