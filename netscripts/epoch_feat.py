...
class TrainValLoop:
    def __init__(
        ...
        if resume is not None:
            ...
            self.start_epoch = modelio.load_checkpoint_by_name(self.sf_encoder, resume_path=resume[0], state_dict_name="sf_encoder_state_dict", strict=False, device=dist_util.dev())
            ...
            self.start_epoch = modelio.load_checkpoint_by_name(self.traj_decoder, resume_path=resume[0], state_dict_name="traj_decoder_state_dict", strict=False, device=dist_util.dev())
    ...
    def epoch_pass(self, epoch, phase, train=True):
        ...
        for batch_idx, sample in enumerate(self.loader):
            ...
            if train:
                ...
                losses_r =  (future_traj_r_loss + vanilla_r_loss + loss_feat_level['mse_r']) * future_valid[:,0]
                losses_l =  (future_traj_l_loss + vanilla_l_loss + loss_feat_level['mse_l']) * future_valid[:,1]
